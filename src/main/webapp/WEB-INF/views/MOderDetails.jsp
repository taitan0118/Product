			...
			<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
				...
				<div class="table-responsive mt-2"
					...
					<table class="table table-light table-hover mt-3 text-center">
					 ...
					 <tbody>
		    <c:forEach items="${oderdetails}" var="odt">
		    	<tr>
			      <th scope="row">${odt.id}</th>
			      <td>${odt.order.id}</td>
			      <td>${odt.order.account.fullname}</td>
			      <td>${odt.product.name}</td>
			      <td>${odt.price}</td>
			      <td>${odt.quantity}</td>
			      <td>${odt.order.account.username}</td>
			      <td>${odt.order.address}</td>
			      <td>${odt.order.createDate}</td>
			   
			      <td>
			      	<a href="/MaN-Category/delete?id=${ca.id}"><i class="bi bi-box-arrow-down text-warning"></i></a>
			      </td>
			    </tr>
		    </c:forEach>
		  </tbody> 
